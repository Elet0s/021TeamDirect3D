...
struct Output
{
    ...
    float4 texcoord_ : TEXCOORD; //TEXCOORD[n]: 텍스쳐의 UV값을 의미하는 시맨틱네임. 텍스쳐좌표를 뜻하는 Texture Coordinate의 줄임말.
    //uint instanceIndex_ : ROWINDEX; 필요 없음.
...
float4 MonsterInstanceRendering_PS(Output _input) : SV_Target0
{
...
Output MonsterInstanceRendering_VSINST(Input _input)
{
    Output result = (Output) 0;
...
DeferredRenderingOutput MonsterInstanceRendering_PSINST(Output _input)
{
    ...
    result.color_ *= Inst_PixelData[_input.instanceIndex_].mulColor_;
    result.color_ += Inst_PixelData[_input.instanceIndex_].plusColor_;
