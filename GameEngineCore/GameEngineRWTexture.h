...
public:	
	static std::shared_ptr<GameEngineRWTexture> Create(const std::string& _name);
	static std::shared_ptr<GameEngineRWTexture> Create();
...
private:
	//순서없는 연결용으로 사용할 텍스쳐들.
	std::vector<std::shared_ptr<GameEngineTexture>> unorderedAccessTexture_;
