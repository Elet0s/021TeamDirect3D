...
std::shared_ptr<GameEngineFBXMesh> GameEngineFBXMesh::Load(const std::string& _Path)
{
...
{
	std::shared_ptr<GameEngineFBXMesh> newRes = CreateNamedRes(_Name);
...
bool GameEngineFBXMesh::ImportBone()
{
	...
	for (size_t Clusterindex = 0; Clusterindex < ClusterArray.size(); Clusterindex++)
	{
		...
		{
			const int PoseCount = scene_->GetPoseCount();
			for (int PoseIndex = PoseCount - 1; PoseIndex >= 0; --PoseIndex)
			{
				fbxsdk::FbxPose* CurrentPose = scene_->GetPose(PoseIndex);

				if (CurrentPose && CurrentPose->IsBindPose())
				{
					scene_->RemovePose(PoseIndex);
					CurrentPose->Destroy();
				}
			}
		...
		GlobalsPerLink[0] = convertMatrix_;
...
void GameEngineFBXMesh::LoadAnimationVertexData(FBXRenderUnitInfo* _DrawData, const std::vector<FbxClusterData>& vecClusterData)
{
	for (auto& clusterData : vecClusterData)
	{
		...
		Bone* pBone = FindBone(_DrawData->VectorIndex, StrBoneName);
		if (nullptr == pBone)
		{
			continue;
		}
