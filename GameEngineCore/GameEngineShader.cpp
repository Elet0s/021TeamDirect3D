	...
	if (std::string::npos != psEntryIndex)
	{
		...
		if (nullptr != pixelShader)
		{
			...
			if (std::string::npos != psInstEntryIndex)
			{
				...
				{
					//인스턴싱 픽셀셰이더 반환형이 float4가 아니라 "DeferredRenderingOutput"라면 gBuffer 렌더타겟의 텍스처에 렌더링하는 픽셀셰이더라는 뜻.
			...
			if (std::string::npos != psReturnType.find("DeferredRenderingOutput"))
			{
				//픽셀셰이더 반환형이 float4가 아니라 "DeferredRenderingOutput"라면 gBuffer 렌더타겟의 텍스처에 렌더링하는 픽셀셰이더라는 뜻.
