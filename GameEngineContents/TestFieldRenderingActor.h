...
private:
	...
	std::vector<TestFieldObjectData> allFieldObjectDataVector_;	//이 액터가 가진 모든 필드오브젝트들의 렌더링 정보.
	//윈도우 안이든 밖이든 상관없이 전부 가지고 있게 한다.

	std::vector<TestFieldObjectData*> renderingFieldObjectDataVector_;	//화면 안에 들어온 필드 오브젝트들의 렌더링 정보.
