...
void TimeActor::SetTimer(std::string& _Text)
{
	...
	Time_m = static_cast<int>(Time_all) / 3600;		// 총 시간을 초로 바꾼뒤에 3600으로 나눠서 분을 구한다
	Time_s = static_cast<int>(Time_all) - (3600 * Time_m);	// 총 시간을 초로 바꾼뒤에 3600 * 분을 하여 초를 구한다.
	Time_all -= static_cast<float>(Time_s);			// 총 시간에서 초를 빼서 소수점 자리만 구한다.
