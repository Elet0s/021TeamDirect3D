...
void KoboldLivesey::Start()
{
	...
	monsterHp_->GetPixelData().mulColor_ = float4::Green; //녹색
	...
	monsterHpMax_->GetPixelData().mulColor_ = float4::Black; //검은색
	...
	monsterHpScore_->SetText(std::to_string(static_cast <int>(monsterInfo_->hp_)), "맑음");
