...
void Shuriken::ProjectileSort(float _deltaTime)
{
	if (targerSerchTimer_ > 3.f)
	{
		...
		{
			if (targetInst_.size() * shuriKenWeaponInfo_.weponConsecutiveAtkNum_ > i) // 타겟수 * 연속공격횟수만큼 투사체 필요
			{
...
void Shuriken::RenderRotate()
{
	if (targerSerchTimer_ > 3.f)
	{
		...
		monsterList_ = Monster::GetMonsterList();
		for (size_t n = 0; n < shuriKenWeaponInfo_.weponConsecutiveAtkNum_; n++)
		{
			size_t m = (targetInst_.size() * n);
			for (size_t i = 0; i < targetInst_.size(); i++)
			{
				...
				projectileGroupList_[i + m].first->GetTransform().SetWorldRotation(0, 0, 0);
				projectileGroupList_[i+m].first->GetTransform().SetWorldRotation(0, 0, -atan2f(Mx - Px, My - Py) * GameEngineMath::RadianToDegree);
				projectileGroupList_[i+m].first->GetTransform().SetWorldRotation(60, 0, 0);
			}
		}
	}
}
	...
	if (targerSerchTimer_ <= 3.0f)
	{
		if (Istarger == true)
		{
			for (size_t n = 0; n < shuriKenWeaponInfo_.weponConsecutiveAtkNum_; n++)
			{
				...
				for (size_t i = 0; i < targetInst_.size(); i++)
				{
					projectileGroupList_[i + m].first->GetTransform().SetWorldMove(referenceVectorList_[i].Normalize3D() * _deltaTime * shuriKenWeaponInfo_.weaponAtkSpeed_);
					projectileGroupList_[i + m].second->GetTransform().SetWorldMove(referenceVectorList_[i].Normalize3D() * _deltaTime * shuriKenWeaponInfo_.weaponAtkSpeed_);
				}
			}
		}
	}
