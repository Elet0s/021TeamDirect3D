...
TestFieldRenderingActor::TestFieldRenderingActor()
	: tileSize_(256.f),
...
void TestFieldRenderingActor::Start()
{
	//Initialize() 함수에서 진짜 초기화를 하므로 여기서 할 게 없다.
	//moveDir_ += {-960.f, 960.f };
}

void TestFieldRenderingActor::Update(float _deltaTime)
{
...
void TestFieldRenderingActor::End()
{
}

void TestFieldRenderingActor::Initialize(
...
void TestFieldRenderingActor::InitializeFieldObjects(
	size_t _totalFieldObjectCount,
	size_t _objectInWindowCount,
	const float4& _totalFieldSize,
	float _diffusionDegree /*= 1.f*/
...
void TestFieldRenderingActor::InitializeFieldRenderer(size_t _objectInWindowCount)
{
	fieldRenderer_ = GetLevel()->GetMainCamera()->GetInstancingRenderer("TestFieldRenderer");
...
void TestFieldRenderingActor::UpdateTilePosition(const float4& _thisWorldPosition)
{
	int unitIndex = 0;
	for (int y = 0; y < tileCountXY_.IY(); ++y)
	{
...
void TestFieldRenderingActor::UpdateFieldObjectInfos(const float4& _thisWorldPosition)
{
	renderingFieldObjectDataVector_.clear();
	//뭐가 들어있든지 일단 전부 비운다. 
	//capacity는 변하면 안된다.

	for (TestFieldObjectData& singleObjectData : allFieldObjectDataVector_)
	{
...
void TestFieldRenderingActor::LoopFieldObject(const float4& _thisWorldPosition)
{
	for (TestFieldObjectData& singleFieldObject : allFieldObjectDataVector_)
	{
